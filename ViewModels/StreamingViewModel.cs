...
public sealed class StreamingViewModel : ViewModelBase
{
    ...
    private string _recommendation = "Actualiza el snapshot antes de salir en vivo.";
    ...
    public StreamingViewModel(AppServices services, Func<OptimizationAction, ActionCardViewModel> cardFactory)
    {
        ...
        [
            ...
            cardFactory(new OptimizationAction { Id = "streaming.game-mode", Name = "Modo juego para streaming", Description = "Activa Game Mode antes de capturar o streamear.", Module = "Streaming", RiskLevel = RiskLevel.Low, IsReversible = true, WhenToUse = "Gaming + stream en la misma PC.", ExpectedImpact = "Windows intenta priorizar la sesion de juego.", CurrentState = "Seguro", ExecuteAsync = services.Gaming.EnableGameModeAsync }),
            ...
            cardFactory(new OptimizationAction { Id = "streaming.flush-dns", Name = "Limpiar DNS", Description = "Refresca resolucion antes de Twitch, YouTube o Discord.", Module = "Streaming", RiskLevel = RiskLevel.Low, WhenToUse = "Si cambiaste DNS o un servicio no resuelve.", ExpectedImpact = "No corta internet.", CurrentState = "Seguro", ExecuteAsync = services.Network.FlushDnsAsync }),
            cardFactory(new OptimizationAction { Id = "streaming.tips", Name = "Reducir tips de Windows", Description = "Evita sugerencias que pueden interrumpir una sesion.", Module = "Streaming", RiskLevel = RiskLevel.Low, IsReversible = true, WhenToUse = "Antes de capturas largas o presentaciones.", ExpectedImpact = "Menos interrupciones visuales.", CurrentState = "Seguro", ExecuteAsync = services.Privacy.DisableTipsAsync }),
            cardFactory(new OptimizationAction { Id = "streaming.startup", Name = "Revisar apps de inicio", Description = "Pendiente: listar apps que arrancan con Windows y sugerir revision.", Module = "Streaming", RiskLevel = RiskLevel.Low, IsImplemented = false, IsEnabled = false, WhenToUse = "Cuando muchas apps abren solas antes de stream.", ExpectedImpact = "No se deshabilitara nada sin confirmacion.", CurrentState = "Pendiente", DisabledReason = "Pendiente en v1.2.0: lectura de Startup/Run con UI de confirmacion y backups." })
        ];
    }
    ...
    public async Task LoadAsync()
    {
        ...
        Recommendation = Snapshot.StreamingApps.Count == 0
            ? "Recomendacion: abri OBS/Streamlabs/TikTok Live Studio y actualiza para priorizar correctamente."
            : "Recomendacion: evita reset de red o limpieza de cache de shaders durante stream. Revisa top procesos si hay tirones.";
    }
    ...
    private async Task<ActionResult> ApplyStreamingProfileAsync(CancellationToken cancellationToken)
    {
        ...
        {
            results.Add(ActionResult.Ok("Prioridades", "No se detecto app de streaming ni juego para priorizar."));
        }
